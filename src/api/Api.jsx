...
export const searchMultiData = async (q) => {
  const search = await axios.get(
    `${baseURL}/search/multi?api_key=${api_key}&query=${q}`
  )
  return search.data
}
...
export const getDetailMedia = async (id, media_type) => {
  if (media_type === 'movie') {
    const movie = await axios.get(`${baseURL}/movie/${id}?api_key=${api_key}`)
    return movie.data
  } else if (media_type === 'tv') {
    const tv = await axios.get(`${baseURL}/tv/${id}?api_key=${api_key}`)
    return tv.data
  } else {
...
export const getStreamingMovies = async () => {
  const movie = await axios.get(
    `${baseURL}/discover/movie?api_key=${api_key}&watch_region=US&with_watch_monetization_types=flatrate`
  )
  return movie.data.results
}
...
export const getForRentMovies = async () => {
  const movie = await axios.get(
    `${baseURL}/discover/movie?api_key=${api_key}&watch_region=US&with_watch_monetization_types=rent`
  )
  return movie.data.results
}
...
export const getInTheatersMovies = async () => {
  const movie = await axios.get(
    `${baseURL}/discover/movie?api_key=${api_key}&region=US&with_release_type=3|2`
  )
  return movie.data.results
}
